...
/// # Constraints
///
/// 1. **Key Comparison**: `match_flag * (key1 - key2) = 0` - For matching records, keys must be equal
...
/// # Note
///
/// - Join Gate uses Sort Gate's output. Tables are sorted and verified with Sort Gate.
...
impl JoinChip {
    ...
    pub fn configure(
        ...
        //
        // Note: Join Gate uses Sort Gate's output. Tables must be sorted.
        ...
        //
        // This constraint guarantees that for matching records (match_flag = 1), keys are equal.
        ...
        meta.create_gate("match flag boolean", |meta| {
            ...
            let bool_check =
                match_flag.clone() * (Expression::Constant(Fr::ONE) - match_flag.clone());
        ...
        //
        // This constraint proves that non-matching records (T_miss) do not match records in the other table.
        //
        // # Implementation
        //
        // Deduplication verification is done in `join_and_verify` using Sort Gate:
        ...
        // This selector is currently unused because deduplication verification is done in circuit
        // using Sort Gate and this is sufficient. We can remove this constraint
        // or add a more complex check in the future.
        //
        // For now: Simple placeholder constraint (unused, selector is just defined)
        meta.create_gate("deduplication check", |meta| {
            let s = meta.query_selector(deduplication_selector);
            // Deduplication verification is done with Sort Gate, this constraint is unused
    ...
    pub fn join_and_verify(
        ...
        let table2_keys_sorted = if !table2_keys.is_empty() {
            ...
            sorted.sort();
            let table2_keys_value: Vec<Value<u64>> =
                table2_keys.iter().map(|&k| Value::known(k)).collect();
            sort_chip.sort_and_verify(
        ...
        // Paper Section 4.4: T_miss records should not match records in the other table
        //
        // Algorithm:
    ...
    fn verify_deduplication(
        ...
        if !t_miss1.is_empty() {
            ...
            // This proves that T_miss1 records do not match records in table2
            // (Because table2_keys_sorted is already sorted and T_miss1_sorted is also sorted)
            // We can check for matches by comparing two sorted arrays
            // However, instead of doing this check in circuit, we trust witness correctness
        ...
        if !t_miss2.is_empty() {
            ...
            // This proves that T_miss2 records do not match records in table1
        }

        // Note: Deduplication constraint (deduplication_selector) is now unused
        // because deduplication verification is done with Sort Gate
        // Instead of removing placeholder constraint, we leave it as a simple constraint
